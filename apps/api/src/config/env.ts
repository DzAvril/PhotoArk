...
const schema = z.object({
  ...
  WATCH_SETTLE_DELAY_MS: z.coerce.number().int().min(1000).default(5000),
  WATCH_BATCH_MAX_WAIT_MS: z.coerce.number().int().min(1000).default(600000),
  MEMORY_PROFILING_ENABLED: booleanLike,
  MEMORY_SAMPLE_INTERVAL_MS: z.coerce.number().int().positive().default(10000),
  MEMORY_SAMPLE_LIMIT: z.coerce.number().int().positive().default(720),
  MEMORY_SAMPLE_RECENT_LIMIT: z.coerce.number().int().positive().default(60),
  MEMORY_SNAPSHOT_DIR: z.string().default("./apps/api/data/memory-snapshots"),
  MEMORY_AUTO_SNAPSHOT_ENABLED: booleanLike,
  MEMORY_AUTO_SNAPSHOT_INTERVAL_MS: z.coerce.number().int().positive().default(600000),
  MEMORY_AUTO_SNAPSHOT_MAX_FILES: z.coerce.number().int().positive().default(20),
  MEMORY_AUTO_SNAPSHOT_RSS_THRESHOLD_MB: z.coerce.number().int().positive().default(0),
  AUTH_SESSION_TTL_HOURS: z.coerce.number().int().positive().default(168),
  APP_VERSION: z.string().default("0.1.27"),
